...
`;
code = code.replace('type AiProvider = "gemini" | "groq";', fetchImageFn + '\ntype AiProvider = "gemini" | "groq";');

// Update AiDiagnosisResponse
code = code.replace(
  'suggestedProductIds?: string[];',
  'suggestedProducts?: string[];'
);

// Replace buildDiagnosisPrompt
...
code = code.replace(/function buildDiagnosisPrompt\([\s\S]*?\n\}/, newPrompt);

// Update analyzeWithGemini
const newGemini = `async function analyzeWithGemini(
  prompt: string,
  userImages: string[],
  ...
  for (let idx = 0; idx < userImages.length; idx++) {
    const base64Data = userImages[idx];
    parts.push({
      inlineData: {
        data: base64Data,
        mimeType: "image/jpeg",
...
code = code.replace(/async function analyzeWithGemini\([\s\S]*?\n\}/, newGemini);

// Update analyzeWithGroq
const newGroq = `async function analyzeWithGroq(
  prompt: string,
  userImages: string[],
  referenceData: ReferenceData[]
): Promise<AiDiagnosisResponse> {
...
code = code.replace(/async function analyzeWithGroq\([\s\S]*?\n\}/, newGroq);

// Update analyzeWithFallback
const newFallback = `async function analyzeWithFallback(
  prompt: string,
  userImages: string[],
  referenceData: ReferenceData[]
): Promise<AiDiagnosisResponse> {
...
code = code.replace(/async function analyzeWithFallback\([\s\S]*?\n\}/, newFallback);

// Update runAiDiagnosis
const newRunAiDiagnosis = `async function runAiDiagnosis(
  diagnosisId: string,
  base64Images: string[],
  cropType?: string,
  growthStage?: string
) {
  console.log(\`[AI Diagnosis Start] ID: \${diagnosisId}, Crop: \${cropType || "N/A"}, Stage: \${growthStage || "N/A"}, Images count: \${base64Images.length}\`);
  
  // 1. Fetch all available products to map suggested names to IDs
  const allProducts = await prisma.product.findMany({
    where: { isActive: true },
    select: { id: true, name: true }
  });

  // 2. Build reference data from planStageDesease
  ...
  try {
    ...
    // Map suggested product names to IDs
...
code = code.replace(/async function runAiDiagnosis\([\s\S]*?^}$/m, newRunAiDiagnosis);
